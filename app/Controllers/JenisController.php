<?php

namespace App\Controllers;

use CodeIgniter\Controller;
use App\Models\Jenis;

class JenisController extends Controller
{

    public function __construct()
    {
        $this->model = new Jenis();
    }

    public function index()
    {
        $data = [
            'main'    => 'page/jenis/list',
            'title'    => 'Data Jenis',
            'list'    => $this->model->orderBy('id', 'desc')->findAll(),
        ];
        return view('page/main', $data);
    }

    public function add()
    {
        $data = [
            'main'    => 'page/jenis/form',
            'title'    => 'Tambah Jenis',
            'action' => '/page/jenis/store',
        ];
        return view('page/main', $data);
    }

    public function store()
    {
        // $data = [
        //     'jenis_value'    => $this->request->getPost('jenis_value'),
        //     'jenis_customer'    => $this->request->getPost('jenis_customer'),
        // ];

        $data = $this->request->getPost();


        $this->model->save($data);
        session()->setFlashData('notif', 'Data berhasil ditambah');
        return redirect()->to('/page/jenis');
    }

    public function update($id = null)
    {
        $data = $this->model->find($id);
        $data = [
            'main'    => 'page/jenis/form',
            'title'    => 'Ubah Tipe Jenis : ' . $data['jenis_customer'],
            'edit_data'    => $data,
            'action' => '/page/jenis/edit/' . $id,
        ];
        return view('page/main', $data);
    }

    public function edit($id = null)
    {
        // $data = [
        //     'price_value'    => $this->request->getPost('price_value'),
        //     'price_type'    => $this->request->getPost('price_type'),
        //     'price_status'    => $this->request->getPost('price_status'),
        // ];
        $data = $this->request->getPost();

        $this->model->update($id, $data);
        session()->setFlashData('notif', 'Data berhasil diubah');
        return redirect()->to('/page/jenis');
    }

    public function delete($id = null)
    {
        $this->model->delete($id);
        session()->setFlashData('notif', 'Data berhasil dihapus');
        return redirect()->to('/page/jenis');
    }
}
